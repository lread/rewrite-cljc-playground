...
This is the main rewrite-cljc API and might very well be all you need.
...
TODO: Finding uses `sexpr` internally and therefore has caveats.
...
TODO: get, assoc and edit use `sexpr` internally and therefore have caveats
...
<3> When we convert to the map node to an sexpr we get the Clojure form which does not include the specifics of the original whitespace
