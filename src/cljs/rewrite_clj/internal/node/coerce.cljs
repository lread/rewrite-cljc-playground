(ns ^:no-doc rewrite-clj.internal.node.coerce
  (:require [clojure.string :as string]
            ...
            [rewrite-clj.internal.node.uneval :refer [UnevalNode]]
            [rewrite-clj.internal.node.meta :refer [MetaNode meta-node]]
            [rewrite-clj.internal.node.fn :refer [FnNode]]
            [rewrite-clj.internal.node.protocols :refer [NodeCoerceable coerce]]
            [rewrite-clj.internal.node.reader-macro :refer [ReaderNode ReaderMacroNode DerefNode reader-macro-node var-node]]
            [rewrite-clj.internal.node.seq :refer [SeqNode vector-node list-node set-node map-node]]
            [rewrite-clj.internal.node.token :refer [TokenNode token-node]]
            [rewrite-clj.internal.node.whitespace :refer [WhitespaceNode NewlineNode whitespace-node]]
            [rewrite-clj.internal.node.whitespace :as ws]))
